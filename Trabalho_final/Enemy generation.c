...
void draw_sprite_partial(int y, int x, uint16_t sprite[][11])
{
    for(int r=0;r<SQ_H;r++){
        ...
        for(int c=0;c<SQ_W;c++){
            ...
            if(color) ppix[base+xx] = color;
        }
    }
}

void delay(){
    volatile int i; for(i=0;i<6000;i++);
}
...
int main()
{
    ...
    while(1){
                ...
                if(!conflict(y0,x)){
                    if(ecount<MAX_ACTIVE){
                        ...
                        if(enemies[ecount].type==1)
                            draw_sprite_partial(y0,x,inimigo1_colors);
                        else
                            draw_sprite_partial(y0,x,inimigo2_colors);
        ...
        for(int i=0;i<ecount;i++){
            if(enemies[i].type==1)
                draw_sprite_partial(enemies[i].y,enemies[i].x,inimigo1_colors);
            else
                draw_sprite_partial(enemies[i].y,enemies[i].x,inimigo2_colors);
        }
